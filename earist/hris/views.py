...
# @login_required(login_url='login')
@allowed_users(allowed_roles=['EMPLOYEE','FACULTY'])
def u_upload_file(request):
    if request.method == 'POST':
        form = FileUploadForm(request.POST, request.FILES)
        if form.is_valid():
            uploaded_file = request.FILES['file']
        ...
        form = FileUploadForm()
    return render(request, 'hris/user_upload2.html', {'form': form})
...
def user_page(request):
    ...
    return render(request, 'hris/user_page.html',)
...
def u_accountSettings(request):
    ...
    context = {'form': form}
    return render(request, 'hris/user_account_settings.html', context)
...
#----------------------------------------COMPUTE HONORARIUM----------------------------
        ...
        if break_out_time and time_out_time != time_object:
            ...
            # print(difference_afternoon,"difference_afternoon")
        else:
...
# Combine the current date with the midnight time
   ...
   #----------------------------------------COMPUTE HONORARIUM----------------------------
        ...
        difference_morning_pattern =break_starts -  datetime.combine(datetime.today(), official_office_in_datetime)
        difference_afternoon_pattern = datetime.combine(datetime.today(), official_office_out_datetime) - break_end

        tardiness_morning_nonacad =   difference_morning_pattern - difference_morning
        tardiness_afternoon_nonacad = difference_afternoon_pattern - difference_afternoon
        tardiness_total_hours = tardiness_morning_nonacad + tardiness_afternoon_nonacad 
        difference_morning_pattern = difference_morning_pattern.seconds % 3600 

        tardiness_total_minutes = (tardiness_total_hours.seconds % 3600) // 60
        tardiness_total_seconds = (tardiness_total_hours.seconds % 60) 
        tardiness_total_hours = (tardiness_total_hours.seconds // 3600) + deduction_time
        ...
        print(tardiness_total_seconds,"tardiness_total_seconds")
...
def search_records(request):
    employeeNumber =  request.user.employee
    if request.method == 'POST':
...
def u_search_records(request):
    ...
    return render(request, 'hris/user_search.html', {'form': form})


@admin_only
def search_attendance(request):
    employee_num = request.user.employee
    if request.method == 'POST':
        ...
        return render(request, 'hris/search_results.html', {'attendance_records': attendance_records, 'date_range': date_range, 'start_date': start_date, 'end_date': end_date, 'employee':employee, 'employee_num': employee_num,})
...
@allowed_users(allowed_roles=['EMPLOYEE','FACULTY'])
def u_search_attendance(request):
    employee_num = request.user.employee
    if request.method == 'POST':
        employee_id = request.POST.get('employee_id')
        start_date = request.POST.get('start_date')
    ...
    return render(request, 'hris/user_search_attendance.html')
...
def u_edit_logs(request):
    ...
    return render(request, 'hris/user_edit_logs.html', {'logs': logs, 'query': query})